    ...
    if(dm)
    {
        ...
        switch(dm_type)
        {
            ...
            case ALIOT_DM_EVENT:     //事件上报
                ...
                cJSON_AddObjectToObject(dm->dm_js,"params");
                break;
            case ALIOT_OTA_VERSION:
                cJSON_AddObjectToObject(dm->dm_js,"params");
                break;
            case ALIOT_OTA_PROGRESS:
                cJSON_AddObjectToObject(dm->dm_js,"params");
                break;
            default:break;
        }
...
void aliot_set_dm_ota_progress(ALIOT_DM_DES* dm,int step,const char* desc)
{
    if(dm)
    {
        ...
        {
            char step_str[8];
            snprintf(step_str,sizeof(step_str),"%i",step);
            cJSON_AddStringToObject(param_js,"step",step_str);
            cJSON_AddStringToObject(param_js,"desc",desc);
        }
    }
}
